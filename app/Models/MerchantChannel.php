...
//            }
    ...
    {
        global $app;
        return (float) $app->getContainer()->redis->get('merchantChannel:tc:' . date('Ymd') . ':' . $merchantNo . ':' . $channelMerchantNo . ':' . $payType . ':' . $bankCode . ':' . $cardType);
    }

    public function getCacheByDayNumLimit($merchantNo, $channelMerchantNo, $payType, $bankCode, $cardType)
    {
        global $app;
        return (float) $app->getContainer()->redis->get('merchantChannel:c:' . date('Ymd') . ':' . $merchantNo . ':' . $channelMerchantNo . ':' . $payType . ':' . $bankCode . ':' . $cardType);
    }

    public function incrCacheByDayAmountLimit($merchantNo, $channelMerchantNo, $payType, $bankCode, $cardType, $payMoney)
    {
        global $app;
        $num = (float) $app->getContainer()->redis->incrby('merchantChannel:tc:' . date('Ymd') . ':' . $merchantNo . ':' . $channelMerchantNo . ':' . $payType . ':' . $bankCode . ':' . $cardType, $payMoney);
        $app->getContainer()->redis->expire('merchantChannel:tc:' . date('Ymd') . ':' . $merchantNo . ':' . $channelMerchantNo . ':' . $payType . ':' . $bankCode . ':' . $cardType, 86400);
        return $num;
    }
    ...
    {
        global $app;
        $num = (float) $app->getContainer()->redis->incrby('merchantChannel:c:' . date('Ymd') . ':' . $merchantNo . ':' . $channelMerchantNo . ':' . $payType . ':' . $bankCode . ':' . $cardType, $num);
        $app->getContainer()->redis->expire('merchantChannel:c:' . date('Ymd') . ':' . $merchantNo . ':' . $channelMerchantNo . ':' . $payType . ':' . $bankCode . ':' . $cardType, 86400);
        return $num;
    }
    ...
    public function getCacheByMerchantNo($merchantNo)
    {
        ...
        $data = $redis->get("merchantChannel:n:" . $merchantNo);
    ...
    public function getCacheByMerchantId($merchantId)
    {
        global $app;
        $redis = $app->getContainer()->redis;
        $data = $redis->get("merchantChannel:i:" . $merchantId);
        return $data ? json_decode($data, true) : [];
    }
    ...
    public function refreshCache($param = [])
    {
        ...
        foreach ($merchant as $v) {
            ...
            if (!empty($data)) {
                $data = $data->toArray();
                $redis->setex("merchantChannel:n:" . $v->merchantNo, 30 * 86400, json_encode($data, JSON_UNESCAPED_UNICODE));
                $redis->setex("merchantChannel:i:" . $v->merchantId, 30 * 86400, json_encode($data, JSON_UNESCAPED_UNICODE));
            }
        }
    }
}
