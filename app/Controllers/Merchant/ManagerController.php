...
class ManagerController extends MerchantController
{
    ...
    //付款人信息变更
    public function changepayer(Request $request, Response $response, $args){
        $payername = Merchant::where('merchantId', $_SESSION['merchantId'])->value('payerName');
        return $this->c->view->render($response, 'merchant/changepayer.twig', [
            'appName' => $this->c->settings['app']['name'],
            'userName' => $_SESSION['userName'] ?? '',
            'menus' => $this->menus,
            'payerName'=> $payername,
    ...
    public function doChangepayer(Request $request, Response $response, $args){
        $payername = $request->getParam('payername');

        $validator = $this->c->validator->validate($request, [
            'payername' => Validator::stringType()->length(1, 80)->noWhitespace()->notBlank(),
        ]);
...
//        Merchant::where('merchantId', $_SESSION['merchantId'])->update(['payerName' => $payername]);
