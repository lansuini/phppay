...
//        print_r($res);exit;
    ...
    public function sendRechargeCodeByMobile($orderData , $param){
    //        $orderData['platformOrderNo'] = 't20190628190106089362';
    //        $param['mobileNo'] = '13552535506';
    //        $param['name'] = '全渠道';
    //        $param['code'] = '000000';
...
//        echo $this->gateway.'/ppi/merchant/itf.do?' . $this->arrayToURL($params);
    ...
    public function withholdMoney($orderData,$param){
        ...
        $params = [
            'Version' => '2.0.0',
            'MerchantId' => $this->params['cId'],
            'IndustryId' => $param['code'] == '000000' ? '11' : '15',
            'MerchOrderId' => $orderData['platformOrderNo'],
...
//        print_r($res->body);exit;
        $this->logger->debug('笔笔快捷', ['url'=>$this->gateway.'/ppi/merchant/itf.do?' . $this->arrayToURL($params)]);
        $res = $this->parseXML($res->body);
...
//        print_r($params);exit;
    ...
    public function rechargeMoney($orderData,$param){
        ...
        $params = [
            ...
            'TradeTime' => date('YmdHis'),
            'ExpTime' => '',
            'NotifyUrl' => $this->getRechargeCallbackUrl($orderData['platformOrderNo']),
            'ExtData' => "",
            'MiscData' => "{$param['mobileNo']}|||{$param['name']}|{$param['idCard']}|{$param['bankCard']}|||||||||",
            'NotifyFlag' => 0,
            'SmId' => md5($orderData['platformOrderNo']),
...
//        print_r($params);exit;
    ...
    //充值专用
    public function rechargeView($orderData){
        //返回发送验证码接口  与 提交扣款接口
        $s1 = getenv('GATE_DOMAIN') . '/paySpecial/sendRechargeCodeByMobile/' . $orderData['platformOrderNo'];
        $s2 = getenv('GATE_DOMAIN') . '/paySpecial/rechargeMoney/' . $orderData['platformOrderNo'];
        return ['view' => 'bank' , 'data' => ['sendCode' => $s1 , 'submit' => $s2 , 'order' => $orderData]];
    }
    ...
    public function getSettlementOrder($orderData)
    {
        ...
        $params['TRANS_DETAILS']['SN'] = $orderData['merchantOrderNo'] ? $orderData['merchantOrderNo'] : 'Y'.$orderData['platformOrderNo'];
...
//        print_r($result);
        ...
        $this->logger->debug("发起代付订单{$orderData['platformOrderNo']}回复：", $returnParams);
        $pay_state = empty($returnParams['TRANS_DETAILS']['TRANS_DETAIL']['PAY_STATE']) ? '': $returnParams['TRANS_DETAILS']['TRANS_DETAIL']['PAY_STATE'];
        if($returnParams['TRANS_STATE'] !== '0000'|| ($pay_state !== '0000' &&  $pay_state !== '00A4'&&  $pay_state !== '0094')){
            $output['status'] = 'Fail';
...
//        print_r($result);
        ...
        $receiveXml = openssl_decrypt(base64_decode($resultdata[0]),'DES-EDE3', $this->des_key,OPENSSL_PKCS1_PADDING);
        $returnParams = $this->xmlToArray($receiveXml);
        $this->logger->debug("查询代付订单 $platformOrderNo 回复 ：", $returnParams);
...
//        print_r($result);
        ...
        $returnParams = $this->xmlToArray($receiveXml);
        $this->logger->debug("查询余额回复 ：", $returnParams);
    ...
    public function getEpEBankRechargeOrder($rechargeData){
        ...
        return $this->getEBankDnaRechargeOrder($rechargeData,'Ep');

    }

    //个人网银DNA
    public function getPsEBankDnaRechargeOrder($rechargeData){

        return $this->getEBankDnaRechargeOrder($rechargeData,'ps');
    }

    private function getEBankDnaRechargeOrder($rechargeData = [] , $type = 'Ps'){

        if($type == 'Ep'){
            $privatKey = $this->params['settlementRechargePrikey'];//商户私钥
            $MerchantNo = $this->params['settlementRechargeMerchantNo'];//'302020000114'; //商户�?
        }else{
            $privatKey = $this->params['settlementRechargePEBDnaPrikey'];//商户私钥
            $MerchantNo = $this->params['settlementRechargePEBDnaMerchantNo'];//'302020000114'; //商户�?
        }
//        $privatKey = $this->params['settlementRechargePrikey'];//商户私钥
//        $MerchantNo = $this->params['settlementRechargeMerchantNo'];//'302020000114'; //商户号
        $url = $this->params['settlementRechargeUrl'];//请求提交地址
        $Version = '2.0.0'; //接口版本
        $OrderFrom = '12'; //订单来源
        $Currency = 'CNY'; //币种
        $Language = '00'; //语言
        $SynAddress = getenv('MERCHANT_DOMAIN').'/rechargeorder'; //同步返回报文地址
//        $AsynAddress = 'http://cb-test.wldev01.com/settlementRecharge/callback'; //异步返回报文地址
        ...
        $MerchantOrderNo = $rechargeData['platformOrderNo'];//商户系统订单?
//        $AsynAddress = $AsynAddress .'/'. $MerchantOrderNo;
        $AsynAddress = $this->getRechargeCallbackUrl($MerchantOrderNo);
        //组建令牌
        $macClear = '';
        ...
        return  $this->getHtmlToUrl($MerchantOrderNo, $sHtml);
    }

    //个人网银笔笔快捷
    public function getPsEBankRechargeOrder($rechargeData){

        $url = getenv('GATE_DOMAIN') . '/paySpecial/rechargeView/' . $rechargeData['platformOrderNo'];
        return $url;
    }

    //网银dna回调
    private function doEBankDnaRechargeCallback($orderData, $request, $type = 'Ps'){
        ...
        $this->logger->debug('doEBankDnaRechargeCallbackDetail：'. $type . '：' .base64_encode($callbackParams['response_text']));
    ...
    //企业网银dna回调
    public function doEpEBankRechargeCallback($orderData, $request){

        return $this->doEBankDnaRechargeCallback($orderData, $request , 'Ep');
    }

    //个人网银dna回调
    public function doPsEBankDnaRechargeCallback($orderData, $request){
        return $this->doEBankDnaRechargeCallback($orderData, $request , 'Ps');
    }

    public function doPsEBankRechargeCallback($orderData, $request){
        ...
        $this->logger->debug('doPsEBankRechargeCallback：',$params);

        if(!in_array($params['Status'] , ['02','10'])) {

            $return['orderStatus'] = 'Fail';
            $return['failReason'] = '支付失败';
            $return['orderNo'] = $params['MerchOrderId'];
            return $return ;
        }
    ...
    public function doRechargeCallBack($orderData, $request){
        ...
        if($orderData['payType'] == 'EnterpriseEBank'){
            return $this->doEpEBankRechargeCallback($orderData,$request);
        }elseif ($orderData['payType'] == 'PersonalEBank'){
            return $this->doPsEBankRechargeCallback($orderData,$request);
        }elseif ($orderData['payType'] == 'PersonalEBankDNA'){
            return $this->doPsEBankDnaRechargeCallback($orderData,$request);
        }else{
            return [
                'status' => 'Fail',
                'orderStatus' => '',
...
//                  echo $key.':',$item . PHP_EOL;
    ...
    public function uploadCheckFile($date = ''){
        $this->initSettlementParams($this->params);
        $MSG_TYPE = 100004;
//        $MERCHANT_NO = '902020000819';
        $MERCHANT_NO = $this->params['settlementSystemNo'];
        $TRANS_DATE = $date ? $date : date('Ymd', strtotime('-1 day'));
        //组建令牌
//        $privatKey = '0A37D991ED574EE3';//商户私钥
        $privatKey = $this->params['settlementSystemKey'];//商户私钥
        $macClear = '';
        $macClear .= !empty($MSG_TYPE) ? $MSG_TYPE.' ' : '';
        $macClear .= !empty($MERCHANT_NO) ? $MERCHANT_NO.' ' : '';
        $macClear .= !empty($TRANS_DATE) ? $TRANS_DATE.' ' : '';
        $macClear = trim($macClear);
        $mac = md5(strtoupper($macClear)." ".$privatKey);
        $MAC =  strtoupper($mac);
        $url_data = parse_url($this->settlementGateway);
        $down_url = $url_data['scheme'].'://'.$url_data['host']."/download?MSG_TYPE={$MSG_TYPE}&MERCHANT_NO={$MERCHANT_NO}&TRANS_DATE={$TRANS_DATE}&MAC={$MAC}";
        return $down_url;
    }
